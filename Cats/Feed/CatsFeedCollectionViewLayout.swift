//
//  CatsFeedCollectionViewLayout.swift
//  Cats
//
...
class CatsFeedCollectionViewLayout: UICollectionViewLayout {
    var contentBounds = CGRect.zero
    var cachedAttributes = [UICollectionViewLayoutAttributes]()
    
    /// - Tag: PrepareCatsFeedCollectionViewLayout
    override func prepare() {
        ...
        var currentIndex = 0
        var segment: CatViewStyle = .fiftyFifty
        ...
        while currentIndex < count {
            ...
            case .fullWidth:
                segmentRects = [
                    CGRect(x: spacing, y: lastFrame.maxY + 1.0 + spacing, width: cellWidth, height: cellHeight)
                ]
                
            case .doubleHeight:
                segmentRects = [
                    CGRect(x: spacing, y: lastFrame.maxY + 1.0 + spacing, width: cellWidth, height: 2*cellHeight)
                ]
            }
            ...
            for rect in segmentRects {
                if currentIndex < count {
                    let attributes = UICollectionViewLayoutAttributes(
                        forCellWith: IndexPath(item: currentIndex, section: 0)
                    )
